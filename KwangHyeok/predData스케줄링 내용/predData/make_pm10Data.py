...
def make_pm10Model(pm10_sclaer):
    ...
    testData_scaled_tdf = pm10_sclaer.transform(X_test)
...
def pred_pm10(pm10_scaler, pm10Model, temp_avg, tmax, tmin, hum_avg, rain_avg, wind_avg, max_wind, wdeg_avg, predOz, predNo2, predco, predso):
    # 파일 경로를 MEDIA_ROOT를 사용하여 설정
    file_path = os.path.join(MEDIA_ROOT, 'KNN_totalData.csv')
    # CSV 파일 읽기
    df_KNNData = pd.read_csv(file_path)
    ...
    pred_Data = df_KNNData.drop(['PM10'], axis=1)
    ...
    predData_scaled = pm10_scaler.transform(pred_Data)
    
    my_pred = pm10Model.predict(predData_scaled)
    pred = int(my_pred[0])
    return pred