...
def make_coModel(sc):
    ...
    # 일산화탄소 모델
    co_df = df_totalData.drop(columns=['오 존', '이산화질소', '아황산가스'])
    X = co_df.drop(['일산화탄소'], axis=1)
    y = co_df['일산화탄소']
    ...
    startTime = time.time()
    history = co_model.fit(
        X_train, y_train, epochs=20, validation_split=0.2, verbose=1
    )
    print('학습시간 : {0:.3f}'.format(time.time() - startTime))
    return co_model

def pred_co(sc, co_model, temp_avg, temp_max, temp_min, hum_avg, rain_avg, wind_avg, wind_max, wdeg_avg):
    dic = {
        '평균기온(℃)': temp_avg, 
        '최고기온(℃)': temp_max, 
        '최저기온(℃)': temp_min, 
    ...
    # 오존 예측값 반환
    pred1 = co_model.predict(array)
    pred2 = pred1.reshape(-1,)
    co_pred = round(pred2[0], 1)
